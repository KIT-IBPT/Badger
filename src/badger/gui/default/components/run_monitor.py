...
class BadgerOptMonitor(QWidget):
    ...
    def init_plots(self, routine, data=None, run_filename=None):
        ...
        try:
            self.obj_names = [
                next(iter(d)) for d in self.routine["config"]["objectives"]
            ]
        except:
            self.obj_names = []
        try:
            self.var_names = [
                next(iter(d)) for d in self.routine["config"]["variables"]
            ]
        except:
            self.var_names = []
        try:
            self.vranges = np.array(
                [d[next(iter(d))] for d in routine["config"]["variables"]]
            )
        except:
            self.vranges = []
        try:
            if self.routine["config"]["constraints"]:
                self.con_names = [
                    next(iter(d)) for d in self.routine["config"]["constraints"]
                ]
            else:
                self.con_names = []
        except:
        ...
        for i, con_dict in enumerate(constraints):
            ...
            if relation == "GREATER_THAN":
                if value <= thres:
                    return (
                        True,
                        f"{name} (current value: {value:.4f}) is less than {thres}!",
                    )
            elif relation == "LESS_THAN":
                if value >= thres:
                    return (
                        True,
                        f"{name} (current value: {value:.4f}) is greater than {thres}!",
                    )
            else:
                if value != thres:
                    return (
                        True,
                        f"{name} (current value: {value:.4f}) is not equal to {thres}!",
                    )
    ...
    def logbook(self):
        try:
            if self.routine_runner:
                ...
                data = self.data
            fileName = send_to_logbook(routine, data, self.monitor)
            upload_to_elog(routine, data, fileName)  # upload to elog
        except Exception as e:
